...
exports.createOrder = asyncHandler(async (req, res) => {
  ...
  try {
    const invoiceCode = `INV-${crypto
      .randomBytes(6)
      .toString("hex")
      .toUpperCase()}`;
    const transitionCode = `TRANS-${crypto
      .randomBytes(6)
      .toString("hex")
      .toUpperCase()}`;
    // Generate unique order number
    result.orderNumber = `ORD-${crypto
      .randomBytes(6)
      .toString("hex")
      .toUpperCase()}`;
    result.orderDate = new Date();
    // ============================================
    // STEP 1: Calculate cart total price
    // ============================================
    ...
    // ============================================
    // STEP 2: Reduce stock for each product in cart
    // ============================================
    for (const item of cart.items) {
      ...
      if (item.variant) {
        ...
        if (variant.totalSales >= 100) {
          // Example threshold for best-selling
    ...
    if (result.coupon) {
      const couponObj = await couponSchema
        .findOne({
          code: result.coupon,
          isActive: true,
        })
      ...
      if (
        couponObj.minPurchaseAmount &&
        cart.totalPrice < couponObj.minPurchaseAmount
      ) {
        throw new customError(
      ...
      // Check usage limits
      if (
        couponObj.usageLimitTotal &&
        couponObj.totalUsed >= couponObj.usageLimitTotal
      ) {
        throw new customError("Coupon usage limit has been reached", 400);
      }
      if (
        (result.user || result.guestId) &&
        couponObj.usageLimitPerUser &&
        couponObj.usersUsed.includes(result.user)
      ) {
      ...
      if (
        couponObj.applicableProducts &&
        couponObj.applicableProducts.length > 0
      ) {
        for (const item of cart.items) {
      ...
      // Check applicable categories
        ...
        couponObj.applicableCategories &&
        couponObj.applicableCategories.length > 0
      ) {
        for (const item of cart.items) {
          const product = await productSchema.findById(item.product);
          if (
            product &&
            couponObj.applicableCategories.includes(product.category)
          ) {
            isApplicable = true;
            break;
          }
      ...
      // Check applicable subcategories
      else if (
        couponObj.applicableSubCategories &&
        couponObj.applicableSubCategories.length > 0
      ) {
        for (const item of cart.items) {
          const product = await productSchema.findById(item.product);
          if (
            product &&
            couponObj.applicableSubCategories.includes(product.subCategory)
          ) {
            isApplicable = true;
            break;
          }
      ...
      // Check applicable brands
        ...
        couponObj.applicableBrands &&
        couponObj.applicableBrands.length > 0
      ) {
        for (const item of cart.items) {
          const product = await productSchema.findById(item.product);
    ...
    // ============================================
    // STEP 4: Check product delivery charge
    // ============================================
    ...
    result.finalAmount =
      cart.totalPrice - discountAmount + deliveryCharge.amount;
    // ============================================
    // STEP 5: Prepare line items (order details)
    // ============================================
    ...
    if (result.paymentMethod === "COD") {
      ...
      // Clear the cart
      const deletedCart = await cartSchema
        .findByIdAndDelete(cart._id)
        .session(session);
      console.log("Cart cleared:", deletedCart);
      // ============================================
    ...
    // Rollback transaction (automatic with abort)
...
exports.updateOrder = asyncHandler(async (req, res) => {
  ...
  const order = await orderSchema.findByIdAndUpdate(orderId, updateData, {
    new: true,
  });
  if (!order) {
    throw new customError("Order not found", 404);
...
exports.getPendingOrdersForCourier = asyncHandler(async (req, res) => {
  const pendingOrders = await orderSchema
    .find({ status: "PENDING" })
    .populate("user")
    .populate("lineItems.product")
    .populate("lineItems.variant")
