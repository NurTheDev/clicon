...
const variantValidationSchema = Joi.object({
    ...
    color: Joi.string().valid('Red', 'Blue', 'Green', 'Black', 'White', 'Yellow', 'Purple', 'Orange', 'Gray', 'Pink'),
...
const updateVariantValidationSchema = variantValidationSchema.fork([
    "product", "price", "stock", "size", "color", "customColor", "images", "barCode", "QRCode", "alertQuantity"
], schema => schema.optional())

exports.updateVariantValidation = async (req) => {
    try {
        const result = await updateVariantValidationSchema.validateAsync(req.body)
        if (result.product) {
