...
exports.productValidation = async (req) => {
  try {
    if (req.body.variantType === "single") {
      ...
      const hasInvalidImage = req.files.images.some(
        (img) => !validMimeTypes.includes(img.mimetype)
      );
      if (hasInvalidImage)
      ...
      if (!validMimeTypes.includes(req.files.thumbnail[0].mimetype)) {
        throw new customError(
          "Thumbnail must be a jpeg, jpg, png or webp",
          400
        );
      }
...
exports.updateValidation = async (req) => {
  try {
    if (req.body.variantType)
      throw new customError("You can't change variant type", 400);
    const result = await updateSingleProductValidationSchema.validateAsync(
      req.body
    );
    if (req.files && req.files.images && req.files.images.length > 0) {
      //     check image mime type
